...
for arg in "$@"; do
  if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
    cat <<'EOF'
benchmark2/scripts/run_all_gd_benchmarks.sh
Runs G(d) sequentially for small, medium, and large datasets.
...
All other arguments are forwarded to benchmark2/scripts/run_gd_benchmark.sh
...
for bucket in small medium large; do
  ...
  log "Running G(d) for ${bucket}"
  cmd=("${BENCHMARK2_SCRIPTS_DIR}/run_gd_benchmark.sh" --dataset-dir "${dataset_dir}")

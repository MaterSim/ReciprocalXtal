...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Coordinate perturbation sigma in Angstrom. Repeat to override the default "
            "three-level coordinate-noise ladder."
        ),
    )
    parser.add_argument(
        "--lattice-noise",
        ...
        default=None,
        help=(
            "Lattice perturbation sigma. Repeat to override the default "
            "three-level lattice-noise ladder."
        ),
    )
...
def main() -> None:
    ...
    if len(coord_noise_levels) != len(lattice_noise_levels):
        raise ValueError(
            "--coord-noise and --lattice-noise must be repeated the same number of times."
        )
    ...
    metadata = {
        ...
        "query_generation": {
            ...
            "paired_noise_levels": [
                ...
                for coord, lattice in zip(coord_noise_levels, lattice_noise_levels)
            ],
            "seed": args.seed,
            "variant_families": [

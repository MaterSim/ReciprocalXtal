...
for arg in "$@"; do
  if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
    cat <<'EOF'
benchmark2/scripts/run_all_structurematcher_benchmarks.sh
Runs StructureMatcher sequentially for small, medium, and large datasets.
...
All other arguments are forwarded to benchmark2/scripts/run_structurematcher_benchmark.sh
...
for bucket in small medium large; do
  ...
  log "Running StructureMatcher for ${bucket}"
  cmd=("${BENCHMARK2_SCRIPTS_DIR}/run_structurematcher_benchmark.sh" --dataset-dir "${dataset_dir}")

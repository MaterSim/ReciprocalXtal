...
def _descriptor_worker_count(entry_count: int) -> int:
    """Choose a worker count for descriptor precomputation.

    Parallel descriptor construction is opt-in because thread-level NumPy/SciPy
    contention can slow some workloads. Set RECIPROCAL_BENCHMARK_WORKERS to a
    value > 1 to enable it explicitly.
    """
    env_value = os.environ.get("RECIPROCAL_BENCHMARK_WORKERS")
    if not env_value:
        return 1
    try:
        requested = max(1, int(env_value))
        return min(requested, max(1, entry_count))
    except ValueError:
        return 1
    ...
    for reference in references:
        ...
        for epsilon in lattice_noise_levels:
            for sigma_angstrom in coord_noise_levels:
                ...
                combined = make_coordinate_perturbed_structure(
                    lattice_perturbed, sigma_angstrom, rng
                )
                queries.append(
                    make_entry(
                        parent_id=reference.parent_id,
...
def fit_distance_threshold(
    ...
    for threshold in candidate_thresholds:
        ...
        if objective == "recall":
            # Prefer recall first, then precision, then F1. On ties, keep the
            ...
            # Prefer precision first, then recall, then F1. On ties, keep the
            # tighter threshold to avoid unnecessary false positives.
            key = (
                metrics["precision"],
                metrics["recall"],
                metrics["f1"],
            ...
            # When multiple thresholds tie on F1/precision/recall, prefer the
            # more permissive threshold to retain calibration margin and reduce
            # held-out false negatives for exact-transform queries.
            key = (
                metrics["f1"],
                metrics["precision"],
                metrics["recall"],
        ...
        elif objective == "f1_then_recall":
            # Keep F1 as the main criterion, but among similarly good F1
            # solutions prefer higher recall and then more permissive
            # thresholds.
            key = (
                metrics["f1"],
                metrics["recall"],
            ...
            f2 = 0.0 if denom <= 0.0 else (5.0 * precision * recall) / denom
            key = (
                f2,
                recall,
                precision,
...
def compute_pnl_descriptors(
    ...
    *,
    ...
    normalized_rbasis = normalize_rbasis_name(rbasis)
    descriptors: Dict[str, np.ndarray] = {}
    runtimes: Dict[str, float] = {}

    def _compute_one(entry: BenchmarkEntry) -> Tuple[str, np.ndarray, float]:
        recp = RECP(
            dmax=dmax,
            nmax=nmax,
            lmax=lmax,
            rbasis=normalized_rbasis,
        )
    ...
    worker_count = _descriptor_worker_count(len(entries))
    if worker_count <= 1:
        for entry in entries:
            entry_id, descriptor, runtime = _compute_one(entry)
            descriptors[entry_id] = descriptor
            runtimes[entry_id] = runtime
        return descriptors, runtimes

    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        for entry_id, descriptor, runtime in executor.map(_compute_one, entries):
            descriptors[entry_id] = descriptor
            runtimes[entry_id] = runtime
    ...
    *,
    ...
    for query in queries:
        ...
        for ref in candidate_refs:
            ...
            #L1
            distance = float(np.sum(np.abs(query_vector - reference_descriptors[ref.entry_id])))
...
def evaluate_descriptor_pairwise_matching(
    *,
    ...
    calibration_grouping: str = "threshold_group",
    ...
    for bucket, bucket_rows in sorted(rows_by_bucket.items()):
        ...
        for threshold_group in threshold_groups:
            if calibration_grouping == "all_noise_same_parent":
                group_calibration = list(calibration_records)
                if threshold_policy == "strict_medium_loose" and threshold_group == "strict":
                    group_calibration = [
                        record
                        for record in calibration_records
                        if record["query_noise_group"] == "no_noise"
                    ]
            elif calibration_grouping == "threshold_group":
                group_calibration = [
                    record for record in calibration_records if record["threshold_group"] == threshold_group
                ]
            else:
                raise ValueError(f"Unsupported calibration grouping: {calibration_grouping}")
            group_evaluation = [
            ...
            if threshold_policy == "strict_medium_loose":
                if threshold_group == "strict":
                    threshold_objective = "f1_then_recall"
                elif threshold_group == "loose":
    ...
    *,
    ...
    for setting in settings:
        ...
        for query in queries:
            ...
            timeout_count = 0
            candidate_refs = select_reference_pool(query, references)
            noise_group = query_noise_group(query.variant_family, query.variant)
...
def summarize_structure_matcher_results(
    ...
    query_groups: Dict[Tuple[str, str, str], List[dict]] = defaultdict(list)

    for row in pair_rows:
        pair_groups[(row["setting"], row["bucket"], row["query_noise_group"])].append(row)
    for row in query_rows:
        query_groups[(row["setting"], row["bucket"], row["query_noise_group"])].append(row)

    for key in sorted(pair_groups):
        setting, bucket, evaluation_query_family = key

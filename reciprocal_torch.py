...
class RECP:
    ...
    def __init__(self, dmax=6.0, nmax=4, lmax=4, rbasis='chebyshev', res=0.1, sigma=None):
        ...
        self.bessel_zeros_torch = torch.arange(1, self.nmax + 1, dtype=torch.float64) * np.pi
    ...
    def build_hkl_grid(self, cell, pad_factor=1):
        """
        Build a fixed HKL grid from a reference cell.

        This is intentionally done once outside the gradient path and then reused
        during optimization so the structure->descriptor map stays differentiable
        with respect to cell/coordinates, while the reciprocal index set stays
        stable.
        """
        ...
        rec_matrix = np.linalg.inv(cell_np).T
        hkl_index = create_index()
        hkl_max = np.array([1, 1, 1], dtype=int)
        hkl_min = np.array([-1, -1, -1], dtype=int)
        ...
        for index in hkl_index:
            ...
            index = np.asarray(index, dtype=int) * multiple
            for i in range(3):
                hkl_max[i] = max(hkl_max[i], index[i])
                hkl_min[i] = min(hkl_min[i], index[i])

        h0, k0, l0 = hkl_min * pad_factor
        h1, k1, l1 = hkl_max * pad_factor
        h = np.arange(h0, h1 + 1)
        k = np.arange(k0, k1 + 1)
        l = np.arange(l0, l1 + 1)
        ...
        mask = d_hkl < (self.dmax * 1.25)
        return torch.tensor(hkl[mask], dtype=torch.float64)

    def _radial_basis_torch(self, r_grid):
        """
        Differentiable radial basis evaluation for one-shell descriptors.

        The active Pnl code uses only the l=0 radial basis. For the Bessel case,
        that reduces to j_0(x)=sin(x)/x with zeros at n*pi, which we can evaluate
        directly in torch without falling back to SciPy.
        """
        if not isinstance(r_grid, torch.Tensor):
            r_grid = torch.tensor(r_grid, dtype=torch.float64)
        r_grid = r_grid.to(dtype=torch.float64).reshape(-1)

        if self.rbasis == "chebyshev":
            return chebyshev_basis(r_grid[:, None], self.nmax, self.rcut).reshape(-1, self.nmax)
        if self.rbasis == "gto":
            return gto_basis(r_grid[:, None], self.nmax, self.rcut).reshape(-1, self.nmax)

        z = self.bessel_zeros_torch.to(device=r_grid.device, dtype=r_grid.dtype)
        x = r_grid[:, None] * z[None, :] / self.rcut
        sinc = torch.where(
            x.abs() < 1e-10,
            torch.ones_like(x),
            torch.sin(x) / x,
        )
        norm = math.sqrt(2.0) * z / (self.rcut ** 1.5)
        return norm[None, :] * sinc

    def build_reciprocal_from_structure_torch(self, cell, coords, numbers, hkl=None):
        """
        Fully differentiable reciprocal peak amplitudes from torch cell/coords.

        Args:
            cell: (3, 3) Cartesian cell matrix
            coords: (N, 3) Cartesian coordinates
            numbers: (N,) atomic numbers
            hkl: optional fixed HKL tensor of shape (M, 3)

        Returns:
            q_xyz: (M, 3) reciprocal vectors without the 2*pi prefactor, matching
                   the legacy descriptor convention.
            intensities: (M,) differentiable diffraction intensities
            d_hkl: (M,) |q|*2*pi values used by the reciprocal cutoff
        """
        device = cell.device
        dtype = cell.dtype
        if hkl is None:
            hkl = self.build_hkl_grid(cell).to(device=device, dtype=dtype)
        else:
            hkl = hkl.to(device=device, dtype=dtype)

        rec_matrix = torch.linalg.inv(cell).transpose(0, 1)
        q_xyz = hkl @ rec_matrix
        d_hkl = torch.linalg.norm(q_xyz, dim=1) * (2.0 * np.pi)
        s2 = d_hkl.square() / (16.0 * np.pi * np.pi)

        frac = coords @ torch.linalg.inv(cell)
        frac = frac - torch.floor(frac)

        numbers_long = numbers.to(torch.int64)
        sfs = torch.zeros((coords.shape[0], hkl.shape[0]), dtype=dtype, device=device)
        for z in torch.unique(numbers_long):
            z_int = int(z.item())
            symbol = chemical_symbols[z_int]
            coeff_np = np.asarray(ATOMIC_SCATTERING_PARAMS[symbol], dtype=np.float64)
            coeff = torch.tensor(coeff_np, dtype=dtype, device=device)
            tmp = torch.exp(-coeff[:, 1][None, :] * s2[:, None])
        ...
        exps = torch.exp(const * g_dot_rs.to(torch.complex128))
        fs = torch.sum(sfs.to(torch.complex128) * exps, dim=0)
        intensities = fs.real.square() + fs.imag.square()
        I0 = torch.sum(numbers.to(dtype)).square().clamp_min(1e-12)
        intensities = intensities / I0

        scaled = torch.clamp(d_hkl / self.dmax, min=0.0, max=1.0)
        taper = 0.5 * (1.0 + torch.cos(np.pi * scaled))
        soft_cutoff = torch.sigmoid((self.dmax - d_hkl) / 0.05)
        intensities = intensities * taper * soft_cutoff
        return q_xyz, intensities, d_hkl

    def compute_sph_descriptor_torch(self, xyz, intensities, norm=False):
        """
        Fully differentiable one-shell Pnl descriptor.
        """
        if xyz.ndim != 2 or xyz.shape[0] == 0:
            return torch.zeros(self.nmax * (self.lmax + 1), dtype=torch.float64, device=xyz.device)

        radii = torch.linalg.norm(xyz, dim=1)
        safe_r = torch.clamp(radii, min=1e-12)
        weight_sum = intensities.sum().clamp_min(1e-12)
        r_shell = torch.clamp((radii * intensities).sum() / weight_sum, min=0.0, max=self.rcut)
        radial_basis = self._radial_basis_torch(r_shell[None]).reshape(-1)

        rhat = xyz / safe_r[:, None]
        Y = spherical_harmonics(
            list(range(self.lmax + 1)),
            rhat.to(torch.float32),
            normalize=False,
            normalization="norm",
        ).to(dtype=torch.float64)

        a_lm = torch.zeros((self.lmax + 1, self.msize), dtype=torch.float64, device=xyz.device)
        weighted_Y = intensities[:, None] * Y
        offset = 0
        center = self.msize // 2
        for l in range(self.lmax + 1):
            dim = 2 * l + 1
            a_lm[l, center - l : center + l + 1] = weighted_Y[:, offset : offset + dim].sum(dim=0)
            offset += dim

        A = radial_basis[:, None, None] * a_lm[None, :, :]
        descriptor = A.square().sum(dim=2).reshape(-1)
        if norm:
            descriptor = descriptor / descriptor.norm().clamp_min(1e-9)
        return descriptor
    ...
    def compute_sph_torch(self, xyz, v, norm=False):
        ...
        Returns:
            Tensor of shape (nmax * (lmax + 1),) representing the one-shell
            P_{nl} descriptor.
        """
        xyz = np.asarray(xyz, dtype=np.float64).reshape(-1, 3)
        if xyz.shape[0] == 0:
            return torch.zeros(self.nmax * (self.lmax + 1), dtype=torch.float32)
        ...
        # 4. Build one-shell A_nlm and compute P_nl = sum_m |A_nlm|^2.
        A = radial_basis[:, None, None] * a_lm[None, :, :]
        descriptor = np.sum(np.abs(A) ** 2, axis=2).reshape(-1).real.astype(np.float32)
        if norm:
...
if __name__ == "__main__":
    ...
    fig.suptitle("Three-way P_nl comparison", fontsize=12, y=0.98)
